...
## STEP1: Create a file:-
  echo "Hello, this is an encrypted file" > encrypted.txt
    EXPLANATION: (echo) prints text
                 put the message inside double qutes ("")
                 (>) writes output to the file after it
...
## STEP3: Decrypt the file:-
 openssl enc -d -aes-256-cbc -pbkdf2 -in encrypted.enc -out decrypted.txt
   EXPLANATION: the decryption command is similar to the encryption, with a few differences
